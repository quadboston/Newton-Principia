(function(){
const { ns, nspaste, fconf, sconf, sf, stripp,
        tpelect, stdMod, rg, originalPoints,
      } = window.b$l.atree({ ssFList: {
        init_conf
}});
return;


function init_conf(){
    //************************************************
    // //\\ geometical scales
    //************************************************
    //for real picture if diagram's picture
    //is supplied or
    //for graphical-media work-area if not supplied:
    var medWidth = 841;
    var medHeight = 728;

    //to comply standard layout, one must add these 2 lines:
    var medsize = 2 * ( medWidth + medHeight ) / 2;
    var medsizeScale = medsize / sconf.medsize_standard
    //***************************************************************
    // \\// geometical scales
    //***************************************************************

    //====================================================
    // //\\ subapp regim switches
    //====================================================
    sconf.enableStudylab            = false;
    sconf.enableTools               = true;
    //====================================================
    // \\// subapp regim switches
    //====================================================


    //***************************************************************
    // //\\ decorational parameters
    //***************************************************************
    sconf.default_tp_lightness = 30;
    fconf.ESSAY_FRACTION_IN_WORKPANE = 0.5;

    //making size to better fit lemma's diagram
    sconf.LETTER_FONT_SIZE_PER_1000 = 30;

    //overrides "global", lemma.conf.js::sconf
    sconf.pointDecoration.r= 3;

    //--------------------------------------
    // //\\ do override engine defaults,
    //      in expands-conf.js,
    //--------------------------------------
    default_tp_stroke_width = Math.floor( 6 * medsizeScale ),
    defaultLineWidth        = Math.floor( 1 * medsizeScale ),
    handleRadius            = Math.floor( 3 * medsizeScale ),
    // //\\ principal tp-css pars
    //      see: topics-media-glocss.js
    //this makes hanle's border nicely thin
    sconf.nonhover_width    = Math.max( 1, Math.floor( 1*medsizeScale/1.6 ) );
    sconf.hover_width       = Math.max( 2, Math.floor( 7*medsizeScale/1.6 ) );

    //make effect apparently only for line-captions,
    //not for point-captions bs
    //misses: pnameLabelsvg).addClass( 'tp-_s tostroke' );
    //In effect only if tp svg has class 'hover-width'.
    sconf.text_nonhover_width   = 1;
    sconf.text_hover_width      = 2;
    // \\// principal tp-css pars
    //--------------------------------------
    // \\// do override engine defaults,
    //--------------------------------------
    //***************************************************************
    // \\// decorational parameters
    //***************************************************************

    sf.allLettersAreHidden = true;

    //***************************************************************
    // //\\ geometics parameters
    //***************************************************************
    //=============================================
    // //\\ points reused in config
    //=============================================
    //model's spacial unit expressed in pixels of the picture:
    //vital to set to non-0 value
    var mod2med = 360;
    var C = [409, 408 ];
    var medposOfModOrigin_x = C[0]; //for model's axis x
    var medposOfModOrigin_y = C[1]; //for model's axis y

    //----------------------------
    // //\\ sconf.curve
    //----------------------------
    sconf.curve = stripp({
        ellipseA: 1.07,
        ellipseB: 0.86,
        PparT: 0.250 * Math.PI,
        curveParA:  -0.64,
        curveParFi0: 0.0 * Math.PI,
        curveParFiMax: 2 * Math.PI,
    });
    {
        // gets ellipse parameters
        let ellB2 = sconf.curve.ellipseB*sconf.curve.ellipseB;
        let ellA2 = sconf.curve.ellipseA*sconf.curve.ellipseA;
        let excentris2 = 1 - ellA2/ellB2;
        let excentris = Math.sqrt( excentris2 );
        sconf.curve.ellipseFocus = Math.sqrt( ellA2 - ellB2 );
    }
    //----------------------------
    // \\// sconf.curve
    //----------------------------

    //interval of t to construct an arc for Newton's sagitta
    var sForSagitta_valQ = 0.36;

    var P = [0, 0 ]; //set bu sconf.PparT
    var Q = [0, 0 ]; //set in amode8captures

    sconf.diagramOrigin = [ 0, 0 ];
    //=============================================
    // \\// points reused in config
    //=============================================

    //-----------------------------------
    // //\\ topic group colors,
    //      todm: possibly proliferation
    //-----------------------------------
    var given   = [0,     150, 0,      1];
    var proof   = [0,     0,   255,    1];
    var result  = [200,   40,  0,      1];
    var semiaxes= [200,   40,  200,    1];
    var body    = [0,     150, 200,    1];

    var center  = [100,   50,  0,      1];

    var hidden  = [0,     0,   0,      0];
    var context = [0,     0,   0,      1];
    var invalid = [200,  150,  0,      1];
    var curvature  = [200,   40,  200, 1];
    nspaste( tpelect, { //need deep copy
        given,
        proof,
        result,
        center,
        hidden,
        context,
        curvature,
        semiaxes,
        body,
        force   : result,
    });
    //-----------------------------------
    // \\// topic group colors,
    //-----------------------------------

    //---------------------------------------------------
    // //\\ points to approximate and draw original curve
    //---------------------------------------------------
    /*
        //apparently this is not enough, need following in study-model.js
            //except point P which will be user-slided along curve,
            //merges selected points with controls points
            var cPivots = ssD.cpivots;
            //merges positions to help d8d
            rg.a.pos = cPivots[0].rgX.pos;
            rg.c.pos = cPivots[2].rgX.pos;
    */
    stripp( originalPoints, {
        O : {
            pcolor : context,
            //caption : 'C',
            pos: C,
            letterAngle : 120,
            letterRotRadius : 35,
        },

        //-----------------------------------------
        // //\\ Book's prop. 11
        //-----------------------------------------
        E : {
            pcolor : body,
            letterAngle : 90,
        },
        H : {
            pcolor : proof,
            letterAngle : -90,
        },
        I : {
            pcolor : proof,
            letterAngle : 90,
        },
        //-----------------------------------------
        // \\// Book's prop. 11
        //-----------------------------------------



        B : {
            pcolor : proof,
            letterAngle : 90,
        },

        BB : {
            letterAngle : 90,
            undisplayAlways : true,
            doPaintPname : false,
        },


        A : {
            pcolor : semiaxes,
        },

        AA : {
            undisplayAlways : true,
            doPaintPname : false,
        },


        D : {
            pcolor : proof,
            letterAngle : 70,
        },

        K : {
            pcolor : proof,
            letterAngle : 70,
        },

        G : {
            pcolor : proof,
            letterAngle : 70,
        },


        T : {
            pcolor : proof,
            letterAngle : 180,
        },

        R : {
            pcolor : proof,
            letterAngle : 45,
        },


        Z : {
            pcolor : body,
            letterAngle : 45,
            undisplayAlways : true,
            doPaintPname : false,
        },


        Zminus : {
            caption : 'Z',
            pcolor : body,
            letterAngle : 45,
            //undisplay : true,
        },

        /*
        V : {
            pcolor : proof,
            letterAngle : -45,
        },
        */

        v : {
            caption : '𝑣',
            pcolor : proof,
            letterAngle : -45,
            letterRotRadius : 15,
        },

        F : {
            pcolor : proof,
            letterAngle : -135,
        },

        VV : {
            caption : 'V',
            pcolor : proof,
            letterAngle : -45,
        },


        //center of instant curvature circle
        C : {
            pos : C,
            caption : 'Rc',
            pcolor : curvature,
            letterAngle : -45,
            undisplayAlways : true,
            doPaintPname : false,
        },

        //----------------------------------------
        // //\\ Prop. 10 Book's "another solution"
        //----------------------------------------
        u : {
            caption : '𝑢',
            pcolor : proof,
            letterAngle : -45,
            letterRotRadius : 15,
        },

        tCircleCenter : {
            pos : C,
            caption : "C'",
            pcolor : curvature,
            letterAngle : -45,
        },
        //----------------------------------------
        // \\// Prop. 10 Book's "another solution"
        //----------------------------------------



        //----------------------------------------
        // //\\ Prop. 11
        //----------------------------------------
        x : {
            caption : "𝑥",
            pcolor : proof,
            letterAngle : 100,
            letterRotRadius : 20,
        },
        //----------------------------------------
        // \\// Prop. 11
        //----------------------------------------

        //---------------------------------------
        // //\\ draggable points
        //---------------------------------------
        FF : {
            caption : "F",
            pcolor : given,
            letterAngle : -115,
            letterRotRadius : 35,
        },
        S : {
            pcolor : center,
            letterAngle : 90,
            letterRotRadius : 35,
            draggableX  : true,
            draggableY  : true,
        },
        P : {
            pos: P,
            pcolor : body,
            letterAngle : 70,
            draggableX  : true,
            draggableY  : true,
        },
        Q : {
            pos: Q,
            pcolor : proof,
            letterAngle : 225,
            letterRotRadius : 40,
            draggableX  : true,
            draggableY  : true,
        },
        //---------------------------------------
        // \\// draggable points
        //---------------------------------------
    });
    var linesArray = [
        { 'A,AA' : { pcolor : semiaxes }, },
        { 'B,BB' : { pcolor : semiaxes }, },
        //-----------------------------------------
        // //\\ Book's prop. 11
        //-----------------------------------------
        { Qx : { pcolor : proof }, },
        { PH : { pcolor : proof }, },
        { HI : { pcolor : proof }, },
        { OS : { pcolor : center, vectorTipIx : 0, caption : 'h', fontSize : 35,
                    captionShiftNorm : -18, }, },
        { OH : { pcolor : proof }, },
        { PI : { pcolor : proof }, },
        //-----------------------------------------
        // \\// Book's prop. 11
        //-----------------------------------------
        //{ 'CV' : { pcolor : curvature }, },
        { 'PC' : { pcolor : proof }, },
        { 'SP' : { pcolor : center, vectorTipIx : 1,
                    caption : 'r𝛙', captionShiftNorm : 18,  }, },
        { 'P,Zminus' : { pcolor : body }, },
        { 'PZ' : { pcolor : body, vectorTipIx : 1, captionShiftNorm : -18,
                    fontSize : 35, caption : 'u', },
                    },
        { 'ZR' : { pcolor : body }, },
        { 'PR' : { pcolor : body }, },
        { 'QR' : { pcolor : proof }, },
        { 'SQ' : { pcolor : proof }, },
        { 'QT' : { pcolor : proof }, },
        { 'PT' : { pcolor : proof }, },
        { DK : { pcolor : body }, },
        { GP : { pcolor : body }, },
        { Qv : { pcolor : proof }, },
        { Pv : { pcolor : proof }, },
        { Tv : { pcolor : proof }, },
        { vG : { pcolor : proof }, },
        { PF : { pcolor : proof }, },
        { AO : { pcolor : semiaxes }, },
        { DO : { pcolor : proof }, },
        { BO : { pcolor : proof }, },
        { PO : { pcolor : body, vectorTipIx : 0, caption : 'r', fontSize : 35, }, },
        { GO : { pcolor : proof }, },
        { FO : { pcolor : proof }, },
        //Book's "another solution"
        { Tu : { pcolor : proof }, },
        { 'u,VV' : { pcolor : proof }, },
        { uP : { pcolor : proof }, },
        { PQ : { pcolor : proof }, },
        { 'P,VV' : { pcolor : proof }, },
        { 'P,tCircleCenter' : { pcolor : curvature }, },

        { 'FF,O' : { pcolor : given, vectorTipIx : 1, caption : 'c',
                        fontSize : 35, }, },
        { 'S,FF' : { pcolor : center, vectorTipIx : 1, caption : 'q',
                        fontSize : 35,
                        // captionShiftNorm : 18, //already default
                    }, },
        { 'FF,P' : { pcolor : body, caption : '𝗽', captionShiftNorm : -18,
                        vectorTipIx : 1 }, },

        { EP : { pcolor : proof, vectorTipIx : 1, caption : '𝙜', fontSize : 35, } },
        { ES : { pcolor : proof }, },
        { EI : { pcolor : proof }, },
        { EO : { pcolor : proof, }, },
    ];

    ns.paste( sconf, {
        sForSagitta_valQ,

        linesArray,
        medposOfModOrigin_x,
        medposOfModOrigin_y,
        medWidth,
        medHeight,
        mod2med,

        default_tp_stroke_width,
        defaultLineWidth,
        handleRadius,
    });
    sconf.pointDecoration.r = sconf.handleRadius;
    //***************************************************************
    // \\// geometics parameters
    //***************************************************************
    sf.shapesArray= [
        {
            isCurve: true,
            rgn: 'borbit',
                        initShapes: ()=>{
                stdMod.init__Shapes()
            },
            //todm apparently needs implementation
            //cssClass: 'subessay-not-comparing-proof-steps',
        },
        {
            //todm not in need?
            rgn: 'curvatureCircle', //reserves rg spot
        }
    ];
}
})();
