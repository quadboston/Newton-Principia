...
function media_upcreate___part_of_medupcr_basic (){
    ...
    if( rgMainLegend ) {
        ...
        if( amode.logic_phase === 'corollary' ) {
            ...
            switch(amode.subessay) {
                ...
                // the rest are defined in main-legend.js as
                // the array lemma7Data.corollary
                case 'cor-1': visibleRows = [2, 3, 4, 5]; break;
                case 'cor-2': visibleRows = [6, 7, 8, 9, 10, 11, 12, 13];
                break;
                case 'cor-3': visibleRows = [14, 15, 16, 17, 18, 19, 20];
                break;
                default: break;
            }
            rows.forEach((row, index) => {
                row.style.display = visibleRows.includes(index) ?
                    'table-row' : 'none';
            });
        } else {
    ...
    if( amode.subessay === 'sine-derivative' ) {
        var wwLine = ssF.str2line( 'x0,x', 'tp-debug',
                        sconf.lines[ 'x0,x' ], 'Δsin(φ)' );
        //patch: overrides wide-lemma settings
        //for tp-width for svg-text element
