(function(){
    ...
    function media_upcreate___part_of_medupcr_basic (){
        ...
        //-------------------------------------------------
        {
            const pcount = rg.orderedPartPoints.val.length - 1;
            const plim = sconf.DONT_PAINT_BARS_MORE_THAN;
            rg.countNSlider.caption = pcount + ' bases';
            if( pcount > plim ) {
                rg.countNSlider.caption += ' ( ' + plim + ' shown )';
            }
        }
        //-------------------------------------------------
        // \\// corrects sliderN caption
        ...
        ///-------------------------------------------------
        ssF.gcurve2svg({
            fun : rg.leftFunction.dividedDifferences
    ...
    function displayBars(
        ...
        ordPP.forEach( (bar, bix) => {
            ...
            if( left0right === 'left' ) {
                ...
                var p1X         = p1X * D00 + rg.P.pos[0];
                var p2X         = p2X * D00 + rg.P.pos[0];
                var p1Xtop      = p1X + fmin4bar*D10;
                var p2Xtop      = p2X + fmin4bar*D10;
                var p1Y         = fmin4bar * D11 + rg.P.pos[1]; //-rg.A.pos[1];
                var ymin        = rg.P.pos[1];
    ...
    //=========================================================
    /*
    ///good sample when A E P svg filled separatedly
    function fixesZorder (){
        zordered = 2;
        'A E P'.split(' ').forEach( nam => {
            if( zordered === 1 ) return;
            if( !has( rg[nam], 'svgel' )){
                zordered = 1;
                return;
            }
            var svg = rg[nam].svgel;
