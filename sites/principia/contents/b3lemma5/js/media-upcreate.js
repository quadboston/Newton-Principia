( function() {
    var {
        ns, sn, $$, nssvg, has,
        sconf, ssF, ssD, sDomF, tprepo,
        rg, amode, stdMod, toreg,
    } = window.b$l.atree({
        setModule, //delayed till mods loaded
        stdModList : //delayed till mods loaded
        {
        },
    });
    ...
    {
        var pictureLeft = rg.USER_TOUCHED_SCREEN;
        //-------------------------------------------------
        // //\\ colors
        //-------------------------------------------------
        var wwfc        = tprepo;
        ...
        ( function () {
            ...
            toreg( "abscissa" );
            rg.abscissa.svgel = nssvg.polyline({
        ...
        sconf.basePairs.forEach( (bpair,pix) => {
            ...
            if( !has( rg[ x.l5key ], 'model8media_upcreate' )) {
        ...
        if( !has( rg, 'slider-m' ) ) { //todo 'm' is wrong ... do a real flag
