//todm do fit lemma 5 with app framework: sliders, colors, ...
(function(){
...
function init_conf(){
    ...
    var medsizeScale = haz( sconf, 'medsizeScale' ) ||
                        ( medsize / sconf.medsize_standard );

    sf.medposOfModOrigin_x = 98.0;
    var medposOfModOrigin_y = 474;

    sf.medposOfModOrigin_y = medposOfModOrigin_y;

    //fix for the lack of sf.dontRun_ExpandConfig = true;
    sf.medposOfModOrigin_x_original=sf.medposOfModOrigin_x;
    sf.medposOfModOrigin_y_original =sf.medposOfModOrigin_y;

    sf.innerMediaHeight = medHeight + sconf.SLIDERS_LEGEND_HEIGHT;
    ...
    [
        [
            { H : [sf.medposOfModOrigin_x, medposOfModOrigin_y] }, //abscissa
            { A : [0, 159] },                      //ordinate
        ],
        [
            { I : [329, medposOfModOrigin_y] },
            { B : [0, 96] },
        ],
        [
            { K : [563, medposOfModOrigin_y] },
            //C : [561, 157],
            { C : [0, 157] },
        ],
        [
            { L : [781, medposOfModOrigin_y] },
            //D : [783, 345],
            { D : [0, 345] },
        ],
        [
            { M : [1005, medposOfModOrigin_y] },
            //E : [1008, 588],
            { E : [0, 588] },
        ],
        [
            { N : [1236, medposOfModOrigin_y] },
            //F : [1239, 774],
            { F : [0, 774] },
        ],
    ...
    (function() {
        sf.basePairs.forEach( bpair => {
            bpair.forEach( point => {
                ...
                var pos = [ pp[0] - sf.medposOfModOrigin_x, pp[1] - medposOfModOrigin_y ];
    ...
    //to fit app framework
    sn( 'allFlagcss', ssD );

    //todm not automated, prolifer.
    fapp.normalizeSliders( medHeight / 444 );
};
})();
