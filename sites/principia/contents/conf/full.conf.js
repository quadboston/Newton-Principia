...
(function(){
const {ns, url2conf,userOptions,fapp, fconf} = window.b$l.atree({});
...
function full_conf(){
    //it is recommeded to keep this variable in both
    ...
    //===================================================
    // //\\ establishes site id,
    //      aka "principia" in sites/principia
    //===================================================

    //:dont edit, will be replaced by productifier
    let SITEID = "dont edit";

    const siteidMatch = location.pathname.match(
                        /sites\/([^\/]+)\/index\.html$/ );
    if( siteidMatch ){
        SITEID = siteidMatch[1];
    }
    fconf.pathToContentSite     = siteidMatch ? '.' : './sites/'+SITEID;
    fconf.pathToEngineSite      = siteidMatch ? '../..' : '.';
    fconf.pathToContentSiteImg  = fconf.pathToContentSite + '/contents/img';
    fconf.engineImg             = fconf.pathToEngineSite + '/engine-img';
