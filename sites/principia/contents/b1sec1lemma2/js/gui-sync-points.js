...
const { sn, $$, fapp, sconf, fconf, rg, amode, stdMod, } =
      window.b$l.atree({stdModList: { syncPoints, }});
const stdL2       = sn('stdL2', fapp );
const dr          = sn('datareg', stdL2 );
const numModel    = sn('numModel', stdL2 );
const study       = sn('study', stdL2 );
const sdata       = sn('sdata', study );
return;
...
function syncPoint( item )
{
    ...
    var xoff = sconf.medposOfModOrigin_x;
    var yoff = sconf.medposOfModOrigin_y;
    ...
    if( rgid ) {
        ...
        rg[ rgid ].pos[0] = (itemx - xoff) / scale;
        rg[ rgid ].pos[1] = -(iY - yoff) / scale;
        ...
        if( pnameFun ) {
            if( pnameFun === 'a' || pnameFun === 'b') {
                ...
                rg[ pnameFun ].pos[1] =
                    -( numModel.curveFun( itemx ) - yoff ) / scale;
            }
            rg[ pnameFun ].pos[0] = rg[ rgid ].pos[0];
        }
    }
}
...
function syncPoints (){
    ...
    dr.rgCtrlPts.forEach( item => { syncPoint( item ); });
