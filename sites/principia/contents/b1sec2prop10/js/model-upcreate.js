(function(){
const {
        sn, $$, nsmethods, nspaste, nssvg, mcurve, integral,
        mat, has, fconf, ssF, sData, ssD, stdMod, amode, sconf,
        rg, toreg,
...
function model_upcreate (){
    ssF.builds_dq8sagit8displace( {rgOrb:rg.borbit} );
    const dyn_q2xy = rg.borbit.bodyq2xy;
...

    {
        ...
        rg.borbit.graphFW_lemma.wraps_draw_graph(graphArg);
    }
    //------------------------------------------------
    // \\// graph
    //------------------------------------------------
    ...
    //conjugate diameters
    nspaste( rg.G.pos, dyn_q2xy( parQ + Math.PI ) );
    nspaste( rg.D.pos, dyn_q2xy( parQ + Math.PI/2 ) );
    nspaste( rg.K.pos, dyn_q2xy( parQ + 3/2*Math.PI ) );
    ...
    nspaste( rg.A.pos, dyn_q2xy( 0 ) );
    nspaste( rg.AA.pos, dyn_q2xy( Math.PI ) );
    nspaste( rg.B.pos, dyn_q2xy( Math.PI/2 ) );
    nspaste( rg.BB.pos, dyn_q2xy( Math.PI*3/2 ) );
