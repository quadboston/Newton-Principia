...
function completesSlidersCreation(){
    ...
    rg.P.acceptPos = newPos => {
        //---------------------------------------------------
        // //\\ corrects approximate mouse point to
        //      exact point on the circle
        //---------------------------------------------------
        var t =  stdMod.pos2t( newPos );
        if( t < Math.PI*0.01 ||
            t > sconf.curve.curveParFiMax ) {
            t = ( t + sconf.curve.curveParFiMax ) %
                sconf.curve.curveParFiMax;
        } //  return false;
        var newP = rg.borbit.dyn_q2xy( t );
        newPos[0] = newP[0];
        newPos[1] = newP[1];
        //--------------------------------------------------
        // \\// corrects approximate mouse point to
