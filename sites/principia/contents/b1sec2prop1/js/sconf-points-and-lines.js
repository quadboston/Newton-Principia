...
const { sn, eachprop, mapp, nspaste, haz, has, haf, haff,
        fconf, sconf, toreg, rg, ssF, ssD, sDomF, amode,
...
function sconf_points8lines (){
    ...
    ///topic names elected
    var tpel = nspaste( tpelect, { //need deep copy
        diagram,
        speed,
        force,
        forceMove,
        ...
        "kepler-triangle-odd"   : tprepo["kepler-triangle-odd"],
        "kepler-triangle-even"  : tprepo["kepler-triangle-even"],
    ...
    var theor1corollary = fconf.sappId === 'b1sec2prop2' ?
        'logic_phase--none subessay--cor-euc' : 'logic_phase--corollary';
    ...
    Object.keys( decor ).forEach( rgid => {
        const point = decor[ rgid ];
        const pointRack = originalPoints[ rgid ]={};
    ...
    eachprop( decor, (dec,rgid) => {
        ...
        Object.assign( rg[ rgid ], dec );
        Object.assign( dec, rg[ rgid ] );
        rg[ rgid ] = dec;
    });
    //---------------------------------------------------
    ...
    linesConf.forEach( ln => {
        ...
        const rgid = ln.nam[0]+ln.nam[1];
        const lineConf = linesArrElem[ rgid ] = {
            isLine : true,
        };
        //equalizing rg, decor elements and lineConf,
        let rgElem = decor[ rgid ] = sn( rgid, rg );
        rgElem.rgid = rgid;
        ...
        [   'cssClass',
            ...
            has(ln,rgid) && (lineConf[rgid] = ln[rgid]);
            has(ln,rgid) && (rgElem[rgid] = ln[rgid]);
        })
