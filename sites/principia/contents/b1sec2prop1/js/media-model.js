...
function media_upcreate___before_basic (){
    ...
    hafff( rg['sl-rgid-time'], 'upates_timeSlider8unmasksSvg' );
    hafff( rg['sl-rgid-dt'], 'updates_sliderGUI' );
    //***********************************************************
    ...
    const pathFill = sDomF.rgid0arrc_2_rgba( 'path' );
    ['B', 'C', 'D', 'E'].forEach( id => {
            let rgX = rg[ id ];
            if( has(rgX, 'svgel' )){
            ...
            //} else {
                //todo why is this?
                //c cc( rgX.rgid + ' svg is missed in media-model.js' );
            }
            if( POINTS_BCDE_ARE_ACTIVE ) {
...
function media_upcreate___after_basic (){
    ...
    ///when media scales,
    eachprop( stdMod.decor, (dec,rgid) => {
    ...
    if( !zorderFixed ) {
        ...
        path.forEach( (pt, pix) => {
            ...
            const rgid = fkey+'-applied';
            const svg = rg[ rgid ].svgel;
