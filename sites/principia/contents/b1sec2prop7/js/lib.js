(function(){
const {
        sn, $$, nssvg, haz, mat, mcurve, userOptions, ssD,
        ssF, flagdo, sconf, sf, amode, stdMod, rg, sDomF,
...
function curveIsSolvable(){
    ...
    //gives STEPS/FORCE_ARRAY_PERIOD points for graph
    var FORCE_ARRAY_PERIOD = 4;
    //make program simpler as planeCurveDerivatives,
    //but if even we have STEPS = 1 million,
    //it still works, very sturdy,
    ...
    var rrc      = rg.S.pos;
    var solvable_orbit = true;
    var fun = rgCurve.dyn_q2xy;
    {   ////sets initial secrtorial speed as [𝗿𝘃]/2 where v===1
        let { staticSectorialSpeed_rrrOnUU, } =
    ...
    for (var solix=0; solix<=STEPS; solix++ ){
        ...
        if( NON_SOLVABLE_THRESHOLD > cosAbs ) {
            solvable_orbit = false;
            foldPoints.push( [ rr[0], rr[1] ] );
            curvePars.solvablePoint = solvable_orbit;
        }

        ///rebuilds forceGraphArray if yet solvable_orbit
        //if point falls on the graph grid, do the job,
        ///graphArrRem is reminder === 0 for points which
        ///are selected to paint the graph,
        if( !graphArrRem ) {
            var sectSpeedSafe1 =
                1e-100 > Math.abs( staticSectorialSpeed_rrrOnUU ) ?
    ...
    stdMod.P2gix = P2gix;
    flagdo.solvable_orbit = solvable_orbit;
