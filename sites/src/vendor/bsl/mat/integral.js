(function(){
    const {sn, mat,haz} = window.b$l.nstree();
    ...
    */
    ...
    ///Does integrate two functions F and G,
    ///Then compares G with F and synchronazes
    ///partition of G making corresponding areas of
    ///F and G equal.
    ///Calculates a lot of information in
    ///originF, originG.
    ///
    ...
    function equalizeAreas({
        ...
        partition.forEach( ( point, pixDeb ) => {
            ...
            for( var gix=gStartIx; gix<=INTEGRATION_POINTS_LIM; gix++ ) {
                ...
                if( gix === INTEGRATION_POINTS_LIM ) {
                    ...
                    ///area under function G has reached the
                    ///area under function F,
                    ///we save gix in to integG.ix,
                    ///we save x, we save fun,
                    ///and we reset next starting index:
                    ///gStartIx = gix,
                    integG.ix = gix;
                    ...
                    c cc( 'part bar ix='+pixDeb + ' posFx_d='+posFx_d+ '(===)' + rgX.pos[0] );

                    var searchArea_d = posFx_d*posFx_d;
                    c cc( 'search area true=' + searchArea_d.toFixed(4) +
                         ' (===) sa in prog='+ searchAreaG.toFixed(4) );

                    c cc( 'Garea=' + currentArea.toFixed(4) + '(===)' +
                    ...
                    */
                    ...
                    //c cc( pixDeb + ' x='+rgX.integ.f.x.toFixed(4) +
        ...
        function findsMinOnIntervals( f0g ){
            ...
            partition.forEach( (point,pix) => {
                const integX = point.integ[ f0g ];
                if( pix === partition.length - 1 ) {
                    integX.min = integX.fun;
                    return;
                }
                const integX1 = partition[ pix+1 ].integ[ f0g ];
                var startIx = integX.ix;
                const lim     = integX1.ix;
                var min    = integX.fun;
