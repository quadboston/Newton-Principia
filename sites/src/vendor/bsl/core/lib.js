(function(){
    ...
    var { nsmethods } = window.b$l.nstree();
    ...
    nsmethods.stripsExtraSpace = stripsExtraSpace;
    const stripBlankRE = /\s+/g;
    const stripStartRE = /^\s+/;
    const stripEndRE = /\s+$/;
    return;

    //better name:
    //function spaceSeps2singleBlanks( str ){
    function stripsExtraSpace( cssSelector ){
        return (
            cssSelector
                .replace( stripBlankRE, ' ' )
