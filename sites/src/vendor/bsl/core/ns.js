...
//        Checks if there is a conflict in global
//        namespace with this library name.
...
(function(){
    ...
    function setAppNamespace(){
        var uniqueEarthWide =
        'iamniquelks8e00w-e9jalknfnaegha;s[snfs=sieuhba;fkleub92784bna';
        ...
        if( ns ) {
            ...
            ///returns, or clones or copies first-depth-level
            ///into purified object,
            ns.stripp = function(obj, obj2){
                const pure = {__proto__:null};
                if( obj ){
                    if( obj2 ){
        ...
        ///Otherwise, returns defaultProperty.
        ...
        ///If object-property exists, then returns it.
        ///If not:
        ///   If emptyTemplate is given assigns and returns it.
        ///   Otherwise, assigns and returns empty obj with
        ///              __proto__ stripped.
        function sn(
            subname,
            ...
            if( typeof subname === 'object' ){
                //todm remove when fixeed
                ccc( "bug: arg must be a string", subname );
            }
            if(
            ...
            var sns = parentNS[ subname ] =
                typeof emptyTemplate !== 'undefined' ?
                emptyTemplate : {__proto__:null};
            return sns;
...
( function () {
    ...
    {
        if( ns.has( ns, 'd' ) ) return; //creates debugger only once
...
( function() {
    ...
    ///extracts value from obj-path given as string.
    ///not tested:
    ///be aware: very slow
    ns.objpath2value = function( start, path ){
		var tokens = path.split( '.' );
		var len = tokens.length;
		for( var ii = 0; ii < len; ii++ ){
            start = start[tokens[ii]];
        }
        return start;
    }
    ...
    function (
        ...
        if( isArrayPaper && !Array.isArray( wall ) ) {
            ////Paper is array and wall is not. Morthing wall to array,
            ////but preserve its properties.
