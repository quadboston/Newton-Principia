...
function createsGraphFW ( gwrap ){
    ...
    function drawGraph({
        ...
        pix2values.forEach( (points,pix) => {
            if( points.nonSolvablePoint ) return;
            //"undefined", '' skipped
        ...
        if( doSideAxes ) {
            ...
            if( drawAllAxes ) {
                ...
                if( extraAxisWidth > dimX_withMarg/2 ) {
                    plotsCount = Math.floor(
                        (dimX_withMarg/2) / (marginX/2) + 1 );
        ...
        pix2values.forEach( (points,pix) => {
            //"undefined", null, '' skipped
            if( !points || points.nonSolvablePoint ) return;
            var x = points.x;
            var mediaX = marginX +
                            (points.x - xMin) / rangeX * dimX_withMarg;
            if( xRangeFilter ) {
                if( x < xRangeFilter.min ) return;
                if( x > xRangeFilter.max ) return;
            }
            if( branchesEndIx+1 < pix ){
                branches.push([]); //adds new branch
            }
            const pLen1 = branches.length-1;
            //masks makes branches[ yix ] undefined where yix has empty mask
            points.y.forEach( (pointY,yix) => {
                ...
                branches[pLen1][ yix ] = branches[pLen1][ yix ] || [];
                branches[pLen1][ yix ].push( [ mediaX, mediaY ] );
            });
            branchesEndIx = pix;
        });
        //==================================================
        ...
        branches.forEach( branch => {
            branch.forEach( (polyline,plix) => {
                if( !polyline ) return;
                var effStyle = Object.assign( {}, pstyle );
                effStyle.stroke = pix2color[ plix ];
                var plotStyle = haz( plotsPars && plotsPars[ plix ],
                                'plotStyle' );
                if( plotStyle ) {
                    Object.assign( effStyle, plotStyle );
                }
                let svg = haz( polyline, 'svg' );
                svg = polyline.svg = nssvg.polyline({
                    svgel : svg,
                    pivots  : polyline,
                    parent  : gMedia,
                    style   : effStyle,
        ...
        if( plotsPars ) {//curveLabels
            ...
            plotsPars.forEach( (pp,pix) => {//curveLabels
                var ownPolyline = branches[0][ pix ];
                ...
                var xIndex = Math.floor(
                    ( ownPolyline.length - 0.001 ) * pp.fraqX );
                xIndex = Math.min( ownPolyline.length-1, xIndex );
                var [ mediaX, mediaY ] = ownPolyline[ xIndex ];

                let txtarg = {
                    svgel   : !doPaintGridOnlyOnce ?
                        null : haz( pp, 'svgTextEl' ),
                ...
                if( csscls ){
                    ...
                    txtarg['class'] =
                        csscls + ' hover-width tostroke tobold';
                }
        ...
        if( toolLineConfig ) {
            nsmethods.drawToolline({
                ...
                dimB : dimY_withMarg,
                dimA : dimX_withMarg,
                toollineStyle : toolLineConfig.toollineStyle,
                abscissaIxValue : toolLineConfig.abscissaIxValue,
                numberMarks : toolLineConfig.numberMarks,
            });
        }
        return;
    }
