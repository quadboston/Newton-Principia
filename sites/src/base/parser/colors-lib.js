/*
 ...
 following functions do return it:
       ...
       ns.builds_zebraNColors_array (returns array of signatures)
        ...
        (lowrgid__2__glocss8anchorRack builds following:)
...
(function(){
    const {
        ...
        sconf, rg, sDomF, topics, lowrgid_2_glocss8anchorRack,
    } = window.b$l.atree({
        ssFList :
        {
            colorArray_2_rgba,
    ...
    sDomF.rgid2low = nsmethods.camelName2cssName;
    return;
    ...
    ///     { rgb, rgba_low, rgba_high, lowOpacity, highOpacity }
    function lowrgid__2__glocss8anchorRack (){
        eachprop( lowrgid_2_glocss8anchorRack,
    ...
    function colArray_2_flags( gcssRack ){
        ...
        /*
        var cssClass = haz( rgX, 'cssClass' );
        if( cssClass ) {
            //legacy rubbish, rid
            let tofill = !!cssClass.match( /(\s+|^)tofill(\s+|$)/ );
            let tostroke = !!cssClass.match( /(\s+|^)tostroke(\s+|$)/ );
