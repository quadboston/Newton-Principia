(function(){
    ...
    const {
        sn, cssp, $$, eachprop, globalCss, nsmethods, nspaste,
        fapp, sconf, topics, lowrgid_2_glocss8anchorRack,
    } = window.b$l.atree({
        ssFList :
        {
...
${ROOT} svg.graph-box-svg tspan.tp-${low_tpID}.hover-width {
    ...
    function v2_tplinks_2_highlightCss( nextTplinks )
    {
        nextTplinks.forEach( (anrack,tplink_ix) => {
            Object.keys( anrack.rgid2true ).forEach( low_tpID => {
                var Rix = ROOT + '.tp-' + tplink_ix;
                var gcssRack = topics.lowrgid_2_glocss8anchorRack[ low_tpID ];
                var ts = '';
