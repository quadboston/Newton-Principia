...
const {ns, eachprop, exegs, topics, fconf, references, ssD} =
...
///     bodyscript { subessay : "text string, non-structured
///         physical content" }
///
///     essayHeader
///     classStr
...
function exegs_2_frags (){
    //will collect unique keys for aspect, logical_phase,
    //and subessay keynames:
    let allTkeys = {};
    let allAkeys = {};
    let allSkeys = {};
    ...
    eachprop( exegs, ( logic_phaseAspects, logic_phase_id ) => {
        ...
        eachprop( logic_phaseAspects, ( exAspect, aspect_id ) => {
            ...
            exAspect.subexegs.forEach( ( subexeg, exegId ) => {
                ...
                //Elements of this array are atomic fragments
                //which are either text or
                ...
                if( references.text ) {
                    ////todm: why adding ref. text is not controlled
                    ////by fragment itself, but
                    ////happens for each frag?
                    ////references to essay-sources to be cited
                    ////or to be the base of essay
    ...
    eachprop( allTkeys, (prop, kname) => {
        ...
        notCss += (notCss ? ',\n' : '\n') +
        `svg.bsl--svgscene.logic_phase--${kname} .logic_phase-not-${kname}`;
    });
    eachprop( allAkeys, (prop, kname) => {
        hideeCss += (hideeCss ? ',\n' : '\n') +
        `svg.bsl--svgscene.aspect--${kname} .aspect--${kname}`;
        notCss += (notCss ? ',\n' : '\n') +
        `svg.bsl--svgscene.aspect--${kname} .aspect-not-${kname}`;
    });
    eachprop( allSkeys, (prop, kname) => {
        hideeCss += (hideeCss ? ',\n' : '\n') +
        `svg.bsl--svgscene.subessay--${kname} .subessay--${kname}`;
        notCss += (notCss ? ',\n' : '\n') +
        `svg.bsl--svgscene.subessay--${kname} .subessay-not-${kname}`;
    });
    eachprop( ssD.allFlagcss, (prop, kname) => {
        hideeCss += (hideeCss ? ',\n' : '\n') +
        `svg.bsl--svgscene.flagcss--${kname} .flagcss--${kname}`;
        notCss += (notCss ? ',\n' : '\n') +
        `svg.bsl--svgscene.flagcss--${kname} .flagcss-not-${kname}`;
    });
    //masking precedence (masking priority) implies from
    //the sequence of this assignment:
    hideeCss = `
