...
var {
    ...
    fconf, sDomN, ssD, sDomF, ssF, exegs, topics, stripp,
    tpelect, lowrgid_2_glocss8anchorRack,
...
///Inputs: rawActFrValue //type=text string
///
...
///Results in:
///     anchorRack.rgid2true as [ low_tpID ] = true;
///     lowrgid_2_glocss8anchorRack[ low_tpID ]
    ...
    if( topicPreAnchors ) {
        topicPreAnchors.forEach( link => {
            ...
            if(
                ...
                ///plain-words (which become
                ///standart match keys);
                ///collected info will be
                ///used later for first-step of
                ///replacing plain-words with match-clauses,
                ///
                ///in the following second-step replacement,
                ///standart-match-clauses will be normally
                ///parsed to html-anchors as usual "tp-topics",
                collectedDelayedLinks[
                    parsedLink[2] //=topic caption
                ] = {
                    //parsedLink[0] === topic single full clause
                    //ending with ¦ aka
                    //¦anchor config¦anchor caption¦¦,
                    //note: trailing ¦¦ will be stripped,
                    match : parsedLink[0].substring( 0,
                            //"length-1" erases last char which is "¦"
                            //if is ¦IN¦¦¦, then makes ¦IN¦¦,
                            parsedLink[0].length-1
                    ),
                    anchorConfig : parsedLink[1],//tplinkConf
                    replaceeKey : parsedLink[2], //topic caption
                    //anchor caption = first match key
                };
                //todo: what is this about, we don't remember:
                //now do remove this tp-clause from rawActFrValue
            }
            ...
            anchorRack.raw_tpIDs.forEach(
                ...
                if( 'cssbold' === anchorOperator ) {
                    ...
                    anchorRack[ 'tpcolarr' ] = haz( tpelect, match[1] );
                    return;
                }
                //..........................................................
                // \\// checks for reserved topic categories
                ...
                var low_tpID = nsmethods.rgid2low( anchorOperator );
                //anchorRack has collection of anchor commands,
                ///now we do add anchorRack.rgid2tru to anchorRack,
                if( low_tpID  && low_tpID.replace( / /g, '' ) ) {
                    anchorRack.rgid2true[ low_tpID ] = true;
                    //if topicId keyName is missed, then an
                    //empty object created
                    if( !has( lowrgid_2_glocss8anchorRack, low_tpID ) ){
...
function rawFragments2htmlText(
    ...
    eachprop( normalizedActiveFrag, (
        ...
        norFrRack[ akey ] = stripp({
            //text ready to be injected into html dom$
            fragBody:
                //normalizes scriptedAnchors to form
                //<a ... tl-NNNNN dix ...
                fragBody_raw.replace( TOP_ANCH_REG_gu, replWithAnchor ),
    ...
    function replWithAnchor(
        ...
        //.we use tplink_ix_str (may-2026 does not exist, what is this?)
        //.todm ... this is a real bug: tplink_ix does not count
        // all links with
...
///inserts pieces of texts (aka footnotes, comments, ... )
///into essays
...
function insertDelayedBatch( fragBody_raw ) {
    if( Object.keys( collectedDelayedLinks ).length ) {
        ...
        fragBody_raw = fragBody_raw.replace( regEx, function( arg ) {
            ///returns full-replacer = full match which is
            ///  leadingDividor+|config|caption||+trailingDividor,
            ...
            return arguments[1] + //leading spacer
                    ...
                    //match===all===config+key+pipes:
                    //aka: ¦v2graph VSarea¦ABFD¦¦
