(function(){
const {
        sn, nsmethods, $$, nspaste, has, haz, hafff, globalCss, ssF,
        ssD, flagdo, sDomF, sData, amode, stdMod, sconf,
      } = window.b$l.atree({ stdModList: {
        creates_lemma_graph_fw,
        creates_chain_of_containers_under_parent,
        graph_creates_global_css: creates_global_css,
}});
return;
...
function creates_lemma_graph_fw({
    ...
    gwrap.createsToolLineConfig = createsToolLineConfig;
    ...
    function sets_axes (){
        ...
        let n2c = sDomF.rgid0arrc_2_rgba; //name to color
        const c_orbit = n2c( 'borbit' );
        const c_body = n2c( 'body' );
        const c_force = n2c( 'force' );
        const c_sagitta = n2c( 'sagitta' );
        const c_fQR = n2c( 'fQR' );
        const xColor = sData.FORCE_PARAM_IS_ALONG_CURVE ? c_orbit : c_force;
        const axisYLegend = [
            {
        ...
        var axisXLegend = [
            {
                text    :  sData.FORCE_PARAM_IS_ALONG_CURVE ?
    ...
    function makes_mask (){
        ...
        const mask = gcontent.pix2mask;
        mask[0] = true; //instantForce
        mask[1] = solvable_orbit;  //fRQ
            //&& (
            //   subessay === 'corollary1' ||
            //   subessay === 'corollary5'
            //)
        mask[2] = false; //body's speed
        mask[3] = solvable_orbit && TIME; //sagitta
        sconf.SHOW_FORMULAS.forEach( (f,fix) => {
    ...
    ///never used and called as of May, 2026
    ...
    ///in blesson
    function createsToolLineConfig (){
        return {
            toollineStyle : {
    ...
    function wraps_draw_graph({
        ...
        fw.meth.drawGraph({
            ...
            yMax,
                ...
                toolLineConfig : gwrap.createsToolLineConfig(),
                brightenGrid : 0.3,
        });
    }
}

//put is stdMod only for lemma 4
function creates_chain_of_containers_under_parent(
    digramParentDom$, //top ancestor
    gwrap,            //graph framework
){
    gwrap = gwrap || this;
    gwrap.parent = $$.div()
