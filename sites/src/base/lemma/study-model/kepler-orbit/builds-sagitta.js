...
(function(){
const { sn, has, flagdo, stdMod, amode, sconf,
        stripp, rg, ssD, sData,
...
function builds_dq8sagit8displace(pars){ let {
    ulitmacy, rgOrb } = stripp(pars);
    sn( 'solvable_orbit', flagdo, true );

    //well this does not work,
    //the fix must be deeper:
    //if( !flagdo.solvable_orbit ) return;

    const ADDENDUM = amode.aspect === 'addendum';
    const SACC = sconf.SAGITTA_ACCURACY_LIMIT;
    const CR = sconf.CURVE_REVOLVES;
    const QS = sconf.Q_STEPS;
    const bodyq2xy = rgOrb.bodyq2xy;
    ...
    for( let qix=0; qix<=QS; qix++ ) {
        ...
        if( !sconf.TIME_IS_FREE_VARIABLE ){
            var plusQ = bP.q + Dq;
            if( !CR && sconf.curve.orbit_q_end <= plusQ ){
                if( MAKE_RANGE ){
        ...
        const r2 = bP.r2; //rel
        {
            ...
            var plusTix = Math.floor( plusT/tgrid_step );
            if( !flagdo.solvable_orbit ){
                //todo why do we need Math.max(0 when
                //have broken orbit?
                var plusTix = Math.max( 0,plusTix );
            }
            ///todo: why this correction needed?
            if( plusTix >= t2o_len ) {
                plusTix = t2o_len -1;
                ...
                if( tix2orbit.length <= plusTix ){
                    //todo understand the cause and remove
                    ccc( tix2orbit.length, plusTix, tix2orbit[tix2orbit.length-1] );
                    plusTix = tix2orbit.length-1;
                }
        ...
        if( plusQ !== null ){
            const rrplus = bP.rrplus = bodyq2xy( plusQ );
            let minusT = bT - Dt;
            ...
            if( minusTix < 1 ) {
                ...
                ////todm why? because of helps only
                ////in P6 when curve shape changes?
                if( !CR && MAKE_RANGE ){
                    ssD.qix_graph_end = Math.min( ssD.qix_graph_end, qix-1 );
                    bP.invalid = true;
                }
            ...
            //saves data for model-upcreate;
            bP.minusQ = minusQ;
            const rrminus = bP.rrminus = bodyq2xy( minusQ );
