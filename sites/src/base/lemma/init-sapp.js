...
(function(){
const {
        ns, sn, nspaste, has, haz, haff, hafa, eachprop, fmethods,
        sapp, fapp, sconf, fconf, arios, ssF, sDomF, sDomN, ssD,
        exegs, stdMod, amode, rg,
...
function init_sapp (){
    ...
    if( haz(sconf.allLettersAreHidden) ){
        rg.allLettersAreHidden = true;
    }
    //haff( stdMod, 'init_lemma__p6patch' );
    sconf.shapesArray.forEach( gshape => {
        gshape.initShapes(gshape, gshape.rgn);
    });
    haff( stdMod, 'init_lemma' );
