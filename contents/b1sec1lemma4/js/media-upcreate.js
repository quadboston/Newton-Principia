( function() {
    ...
    function displayBars(
        ...
        var CUSTOM_TEST_BAR_IX = -1;  //-1 to disable
        if( CUSTOM_TEST_BAR_IX > -1 ) {
            if( ordBars.length === 5 ) {
                //starts bar sequence over
                testGraph.bars = [];
                testGraph.limitRatio = 1/transformVolume;
                testGraph.zebraColor = ordBars[ CUSTOM_TEST_BAR_IX ].zebraColor;
            }
        }
        ...
        ordBars.forEach( (bar, bix) => {
            ...
            if( left0right === 'left' ) {
                ...
                if( bix === CUSTOM_TEST_BAR_IX ) {
                    var wwRight = transformVolume * barArea;
                    var wwLeft = leftBarsAreas[ bix ];
                    testGraph.bars[ ordBars.length-1 ] = {
                         barsTotal : ordBars.length-1,
                         convergence : barRatio.toFixed(5),
                         barRatio : ( wwLeft/wwRight ).toFixed(5),
                         left : wwLeft.toFixed(5),
                         right :wwRight.toFixed(5),
                    };
                    /*
                    ccc( 'total bars='  + (ordBars.length-1) + ' bar 1: ' +
                         ' left area='  + wwLeft.toFixed(4) +
                         ' right area=' + wwRight.toFixed(4) +
                         ' barRatio='   + barRatio.toFixed(4) +
                         ' limit ratio='+ ( 1/transformVolume ).toFixed(4) +
                         ' color='+bar.zebraColor
                    );
                    */
                }
            }
            ...
            /*
            ///they apparently are not part of tp-machinery, so nothing to do here
            if( bix > 4 ) {
                //todm ... why points still highlight when tplink is hovered
                //leftBottom.undisplayAlways = true;
                //rightBottom.undisplayAlways = true;
                leftBottom.undisplay = true;
                rightBottom.undisplay = true;
            }
            */
            //-------------------------------------------------------
            // \\// establishes registry for bar vertices
            ...
            if( bix === sconf.INDIVIDUAL_BAR_INDEX_IN_LEMMA ) {
                ...
                rgX_bar.svgel.style.stroke = 'red';
                lin.svgel.style.stroke = 'red';
        ...
        if( ordBars.length === 201 && CUSTOM_TEST_BAR_IX > -1 ) {
            //ccc( JSON.stringify( testGraph, null, '  ' ).replace( /$.*null.*\n/gm, '' ));
            ccc( JSON.stringify( testGraph, null, '  ' ) );
        }
    }
    //=========================================================
    // \\// calculates figure, bars areas
