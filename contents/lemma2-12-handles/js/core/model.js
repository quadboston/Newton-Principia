            ...
            curveFW.curveFun = function( x, derivIx ) {
                if( curveFW.integCreated === null ) {
                    ...
                    sconf.ctrlPtXYs_js.forEach( (cp,ii) => {
                        /*
                        if( ii===0 || ii === contLen-1 ) {
                            cp.derivIx = ii===0 ? 0 : len-1;
                            ccc( ii, 'x='+cp.x.toFixed(3), cp.derivIx );
                            return;
                        }
                        */
                        let derivIx = curveFW.x2ix( cp.x );
                        derivIx = Math.min( len-1, Math.max( 2, derivIx ) );
                        cp.derivIx = derivIx;
                        let x = derivIx / len * xRange + x0;
                        cp.x = x;
                        dr.ctrlPts[ii].x = x;
                        dr.ctrlPts[ii].derivIx = derivIx;
                        //c cc( ii, 'x='+cp.x.toFixed(3), cp.derivIx );
                        let y = curveFW.fw.newInteg[ derivIx ];
                        //todm not in two places: garbage:
                        dr.ctrlPts[ii].y = y;
                        cp.y = y;
                        //c cc( ii, 'y='+cp.y.toFixed(3) );
                    });
                }
                if( typeof derivIx === 'undefined' ) {
                    derivIx = curveFW.x2ix( x );
                }
                return curveFW.fw.newInteg[ derivIx ];
            };
        }
    }
